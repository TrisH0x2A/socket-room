...
void gen_token(char *out, size_t byte_len) {
    ...
    if (f) {
        ...
        for (size_t i = 0; i < byte_len; i++)
            snprintf(out + i * 2, 3, "%02x", (unsigned)(rand() & 0xff));
    }
    out[byte_len * 2] = '\0';
}

void gen_salt(char out[33]) { gen_token(out, 16); }

void hash_password(const char *password, const char *salt, char out[65]) {
    char combined[512];
    snprintf(combined, sizeof(combined), "%s:%s", salt, password);
    sha256_hex((const uint8_t *)combined, strlen(combined), out);
}
